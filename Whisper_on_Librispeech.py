...
from transformers import (
    ...
    AutoModelForSpeechSeq2Seq,  #JB added class model for speech to text
    Trainer,
    TrainingArguments,
    DefaultDataCollator,
)
...
from aihwkit.nn.conversion import convert_to_analog, AnalogLinear
from aihwkit.optim import AnalogSGD


# BERT model from Hugging Face model hub fine-tuned on SQuAD v1
MODEL_NAME = "openai/whisper-tiny.en" #JB Change to Whisper Model "openai/whisper-tiny.en" (done)
TOKENIZER = AutoTokenizer.from_pretrained(MODEL_NAME)

# Parse some arguments
PARSER = ArgumentParser("Speech Recongnition using Whisper")
...
PARSER.add_argument("-n", "--noise", help="Modifier noise", default=0.1, type=float) #JB Noise in model
PARSER.add_argument(
    "-r",
    "--run_name",
...
PARSER.add_argument(
    "-l", "--learning_rate", help="Learning rate for training", default=2e-4, type=float #JB Learning rate
)
...
if ARGS.wandb:
    ...
    SWEEP_ID = wandb.sweep(sweep=SWEEP_CONFIG, project="whisper-weight-noise-experiment")

# max length and stride specific to pretrained model
MAX_LENGTH = 320 #JB Check
DOC_STRIDE = 128 #JB Check


def create_ideal_rpu_config(tile_size=512): #Ideal Model
...
def create_rpu_config(modifier_noise, tile_size=512, dac_res=256, adc_res=256):   #PCM Model
...
def create_model(rpu_config):
    ...
    model = AutoModelForSpeechSeq2Seq.from_pretrained(MODEL_NAME) #JB Change (done)
...
def preprocess_train(dataset):
    ...
    tokenized_dataset = TOKENIZER(
        dataset["question"], #JB Change
        dataset["context"], #JB Change
        truncation="only_second", #JB Change
...
def create_datasets():
    """Load theLibrispeech dataset, the tokenized version, and the validation set"""
    libri = load_dataset("librispeech_asr", 'clean') #JB Change (done)

    # Preprocessing changes number of samples, so we need to remove some columns so
    # the data updates properly
    tokenized_data = libri.map(
        preprocess_train, batched=True, remove_columns=libri["train"].column_names
    )
    eval_data = libri["validation"].map(
        preprocess_validation, batched=True, remove_columns=libri["validation"].column_names
    )

    return libri, tokenized_data, eval_data
...
def make_trainer(model, optimizer, tokenized_data):
    ...
    training_args = TrainingArguments(
        output_dir="./",
        save_strategy="no",
        per_device_train_batch_size=32, #JB Changed to 32 from 3
        per_device_eval_batch_size=32, #JB Changed to 32 from 3
        num_train_epochs=30, #JB Changed to 30
        weight_decay=0.005,
        no_cuda=False,
    )
...
def do_inference(model, trainer, libri, eval_data, writer, max_inference_time=1e6, n_times=9):
    ...
    def predict():
        ...
        predictions = postprocess_predictions(
            libri["validation"], eval_data, raw_predictions.predictions
        )
    ...
    ground_truth = [{"id": ex["id"], "answers": ex["answers"]} for ex in libri["validation"]]
...
def main():
    ...
    libri = load_dataset("librispeech_asr", 'clean')
    #libri, tokenized_data, eval_data = create_datasets()
    tokenized_data = LibriSpeech("test-clean")
    ...
    #do_inference(model, trainer, libri, eval_data, writer)
